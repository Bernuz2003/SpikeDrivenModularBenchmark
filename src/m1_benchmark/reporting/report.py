...
def _decision_table(df: pd.DataFrame, pf: pd.DataFrame) -> str:
    ...
    for field, label in [('encoder', 'Encoding'), ('feature_extractor', 'Feature extractor'), ('head', 'Head')]:
        ...
        risk = 'Requires confirmation on full real DVS runs' if str(df['dataset'].iloc[0]).lower() == 'synthetic_dvs' else 'Check robustness and temporal sensitivity before Milestone 2'
