...
def configure_torch_runtime(num_threads: int | None = 1) -> None:
    # Su CPU pochi thread sono spesso piu prevedibili; sul server si puo alzare
    # training.torch_num_threads quando il nodo lo permette.
...
def tensor_is_binary(x: torch.Tensor, tol: float = 1e-6) -> bool:
    if x.numel() == 0:
        return True
    # Tolleranza piccola per convivere con float32 senza accettare valori multi-bit.
