...
def apply_event_augmentations(events: dict[str, Any], cfg: dict[str, Any] | None, rng: np.random.Generator) -> dict[str, Any]:
    ...
    if drop > 0:
        # Drop sugli eventi prima dell'encoding: simula perdita del sensore o
        # filtraggio aggressivo senza cambiare il modello.
    ...
    if jitter > 0 and n > 0:
        # Jitter relativo allo span del sample: piu stabile di un valore assoluto
        # quando i dataset hanno durate diverse.
...
def apply_spike_augmentations(spikes: torch.Tensor, cfg: dict[str, Any] | None, rng: np.random.Generator | None = None) -> torch.Tensor:
    ...
    if cfg.get('timestep_shuffle', False):
        # Shuffle dopo l'encoding: rompe l'ordine temporale lasciando invariata
        # la quantita di spike.
        if rng is None:
    ...
    if early is not None:
        ...
        if 0 < early < spikes.shape[0]:
            # Non tagliamo la shape: azzeriamo la coda cosi il modello vede sempre T costante.
