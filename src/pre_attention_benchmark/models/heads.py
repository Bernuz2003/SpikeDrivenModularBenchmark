...
def pool_spikes(x: torch.Tensor, mode: str = 'max') -> torch.Tensor:
    ...
    For pre-attention benchmark we use max pooling across spatial/token dimensions, preserving
    binary values before the terminal readout.
    """
    if x.dim() == 4:  # [B,T,N,D]
        # Max sui token: se almeno un token spara, la feature resta attiva.
...
class TemporalSpikeVotingReadout(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Accumulo solo al confine terminale: non rientra nella comunicazione hidden.
...
class ClassNeuronAccumulatorHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(currents.shape[1]):
            # Piccolo readout a neuroni di classe; volutamente terminale, quindi
            # puo usare correnti real-valued senza violare i vincoli hidden.
