...
class MetricsCollector:
    ...
    def _make_record(self, name: str, module: nn.Module, inputs: tuple[Any, ...], output: Any) -> LayerMetric | None:
        ...
        return LayerMetric(
            ...
            state_mem_bits_per_sample=state_bits,
    ...
    def _state_bits_per_sample(self, module: nn.Module) -> int:
        ...
        if not shape:
            return 0
        # last_state_shape include il batch del probe PyTorch. Per una stima
        # hardware confrontabile contiamo lo stato di un singolo sample/stream.
        shape = shape[1:]
        n = 1
        for s in shape:
            n *= s
    ...
    def _buffer_bits(self, module: nn.Module, inp: torch.Tensor | None, out: torch.Tensor, is_bin: bool) -> int:
        if isinstance(module, nn.Conv2d) and inp is not None:
            input_is_bin = tensor_is_binary(inp.detach())
            bit = 1 if input_is_bin else 32
    ...
    def _ops(self, module: nn.Module, inp: torch.Tensor | None, out: torch.Tensor, is_bin: bool) -> dict[str, float]:
        ...
        if isinstance(module, nn.Conv2d) and inp is not None:
            ...
            z['mac_dense_ops'] = dense
            z['sops_proxy'] = dense
        elif getattr(module, 'op_class', '') == 'TerminalSpatioTemporalAvgFC':
            z['ac_sparse_ops'] = float(getattr(module, 'last_reduction_ops', out.numel()))
            z['sops_proxy'] = z['ac_sparse_ops']
        elif getattr(module, 'op_class', '') == 'TerminalSpikeVisionSpatialPooling':
            z['ac_sparse_ops'] = float(getattr(module, 'last_reduction_ops', out.numel()))
            z['sops_proxy'] = z['ac_sparse_ops']
        elif getattr(module, 'op_class', '') == 'StatelessSurrogateThreshold':
            z['compare_ops'] = float(out.numel())
            z['sops_proxy'] = z['compare_ops']
        elif getattr(module, 'op_class', '') == 'TerminalClassNeuronAccumulator':
            class_shape = getattr(module, 'last_class_spike_shape', None)
            n_compares = 0
            if class_shape is not None:
                n_compares = 1
                for s in class_shape:
                    n_compares *= int(s)
            z['compare_ops'] = float(n_compares)
            z['sops_proxy'] = z['compare_ops']
    ...
    def _profile_entry(self, name: str, module: nn.Module, inputs: tuple[Any, ...], output: Any) -> dict[str, Any]:
        ...
        entry = {
            ...
            'state_mem_bits_per_sample': self._state_bits_per_sample(module),
        }
...
def summarize_layer_metrics(df: pd.DataFrame) -> dict[str, Any]:
    if df.empty:
        return {
            ...
            'max_state_mem_bits_per_sample': 0,
    ...
    return {
        ...
        'max_state_mem_bits_per_sample': int(grouped['state_mem_bits_per_sample'].max()),
